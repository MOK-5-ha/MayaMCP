...
@patch("src.conversation.processor.Runner.run_async")
...
def test_tip_input_bypasses_intent_detection(mock_scan_output, mock_scan_input, mock_tools, mock_run_async):
    ...
    async def mock_run(*args, **kwargs):
        ...
        yield NS(
            ...
            content=genai_types.Content(role="model", parts=[genai_types.Part.from_text(text="[STATE: happy] Sure, I can add a tip for you.")])
        )
...
@patch("src.conversation.processor.scan_input")
@patch("src.conversation.processor.scan_output")
def test_no_tip_input_triggers_intent_detection(mock_scan_output, mock_scan_input, mock_tools, mock_run_async):
