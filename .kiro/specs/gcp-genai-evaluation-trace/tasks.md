...
- **Acceptance Criteria (BDD)**:
  ...
  Scenario: Dependency validation
    Given the project dependency manifest "requirements.txt"
    When scanned for third-party SaaS tracing libraries
    Then "weave" and "wandb" MUST NOT be present
    And "google-cloud-aiplatform", "google-cloud-trace", and "opentelemetry-exporter-gcp-trace" MUST be specified
  ```
...
- **Acceptance Criteria (BDD)**:
  ```gherkin
  Scenario: Environment variables sanitization
    Given the template file ".env.example"
    When inspecting required environment variables
    Then "WANDB_API_KEY" MUST NOT be present
    And "GCP_PROJECT" and "GCP_LOCATION" MUST be documented for Application Default Credentials
  ```
...
- **Acceptance Criteria (BDD)**:
  ```gherkin
  Scenario: Lock ordering and payment transition
    Given an active session in "completed" payment status
    When an asynchronous background transaction failure occurs
    Then the status MUST transition to "failed" without raising a StateTransitionError
    And the session lock MUST be acquired without holding the global mutex
  ```
...
- **Acceptance Criteria (BDD)**:
  ```gherkin
  Scenario: Non-blocking SSE stream iteration
    Given an active SSE connection to "/v1/chat/stream"
    When streaming chunks are yielded from the ADK runner
    Then the main event loop thread MUST NOT be blocked by generator polling
  ```
...
- **Acceptance Criteria (BDD)**:
  ```gherkin
  Scenario: OpenTelemetry GenAI span attributes
    Given a conversational turn processed by Maya
    When the LLM response completes
    Then an OpenTelemetry span MUST record "gen_ai.system", "gen_ai.usage.input_tokens", and "gen_ai.usage.output_tokens"
  ```
...
- **Acceptance Criteria (BDD)**:
  ```gherkin
  Scenario: Offline telemetry fallback
    Given an offline test environment with no active GCP credentials
    When a conversational span is exported
    Then the system MUST log telemetry locally without throwing an unhandled exception
  ```
...
- **Acceptance Criteria (BDD)**:
  ```gherkin
  Scenario: Vertex AI EvalTask execution
    Given an evaluation dataset of multi-turn drink orders
    When "EvalTask.evaluate()" is invoked with Pointwise and Trajectory metrics
    Then a structured metrics DataFrame MUST be produced containing faithfulness and tool trajectory scores
  ```
...
- **Acceptance Criteria (BDD)**:
  ```gherkin
  Scenario: Qualitative Agent-as-a-Judge scoring
    Given an agent conversational trajectory
    When evaluated by the Antigravity Judge Agent
    Then the judge MUST return a validated "TrajectoryEvaluationRubric" containing integer scores (1-5) and reasoning justifications
  ```
...
- **Acceptance Criteria (BDD)**:
  ```gherkin
  Scenario: Payment failure evaluation
    Given an order amount of "$99.99"
    When the payment tool simulates a background failure
    Then Maya MUST acknowledge the register malfunction and offer recovery options
    And the trajectory score MUST confirm appropriate tool error handling
  ```
...
- **Description**: Remove all remaining Weave / W&B references across [`AGENTS.md`](file:///Users/pretermodernist/Developer/Personal/MayaMCP/AGENTS.md), [`README.md`](file:///Users/pretermodernist/Developer/Personal/MayaMCP/README.md), [`TODO.md`](file:///Users/pretermodernist/Developer/Personal/MayaMCP/TODO.md), and [`CONTRIBUTING.md`](file:///Users/pretermodernist/Developer/Personal/MayaMCP/CONTRIBUTING.md). Document Google Cloud Trace and Vertex AI Evaluation workflows.
- **Acceptance Criteria (BDD)**:
  ```gherkin
  Scenario: Documentation hygiene
    Given the repository documentation files
    When scanned for legacy SaaS tools
    Then zero occurrences of "WANDB_API_KEY", "weave.init", or "Weave" MUST remain in active guidance
  ```
...
- **Acceptance Criteria (BDD)**:
  ...
  Scenario: Full test pass
    Given the modified codebase and test harness
    When "pytest -m 'not slow'" and "ruff check" are run
    Then all tests MUST pass without rate-limit or missing dependency errors
  ```
