...
- [x] 1. Set up security module structure
  - [x] 1.1 Create `src/security/__init__.py` with module exports
    - Export scan_input, scan_output, ScanResult, ScanConfig, is_available
    - _Requirements: 4.1_
  - [x] 1.2 Create `src/security/config.py` with ScanConfig dataclass
...
- [x] 2. Implement core scanner functionality
  ...
  - [x] 2.4 Write property test for prompt injection blocking
    - **Property 1: Prompt injection inputs are blocked**
    - **Validates: Requirements 1.2**
  - [x] 2.5 Write property test for toxic input blocking
    - **Property 2: Toxic inputs are blocked**
    - **Validates: Requirements 2.1**

- [x] 3. Implement output scanning
  - [x] 3.1 Implement scan_output() function
    - Initialize Toxicity output scanner from llm-guard
    - Check toxicity threshold, replace with fallback if exceeded
    - Fail open on any errors
    - _Requirements: 3.1, 3.2, 3.3_
  - [x] 3.2 Write property test for toxic output replacement
...
- [x] 5. Integrate with conversation processor
  - [x] 5.1 Add scan_input call before LLM invocation in processor.py
  ...
  - [x] 5.2 Add scan_output call after LLM response in processor.py
    - Call scan_output on agent_response_text before returning
    - Use sanitized_text from result
    - _Requirements: 3.1, 3.2_
  - [x] 5.3 Write unit tests for processor integration
...
- [x] 6. Add llm-guard dependency
  - [x] 6.1 Update requirements.txt with llm-guard
    - Add llm-guard as optional dependency
    - _Requirements: 4.1_
  - [x] 6.2 Update pyproject.toml with optional security extras
