...
def process_order(
    ...
    with tracer.start_as_current_span("chat.turn") as span:
        ...
        # Capture active OpenTelemetry context for propagation across thread boundary
        parent_otel_context = otel_context.get_current() if otel_context is not None else None
                ...
                try:
                    def _run_coro(coro):
                        ...
                        if loop and loop.is_running():
                            ...
                            def _run_in_thread():
                                tok = None
                                if otel_context is not None and parent_otel_context is not None:
                                    tok = otel_context.attach(parent_otel_context)
                                try:
                                    set_current_session(session_id)
                                    return asyncio.run(coro)
                                finally:
                                    if otel_context is not None and tok is not None:
