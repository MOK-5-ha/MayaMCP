...
- **Centralized Logic**: Extract repetitive code into well-named helper functions following the single responsibility principle. Most utility functions should live in `src/utils/helpers.py` (documented in `docs/helper_functions.md`).
- **Unified LLM Client**: All GenAI calls must go through `src/llm/client.py`. Do not call the Google SDK directly in other modules. Always use `get_genai_client()`. The application operates strictly in **100% GCP Vertex AI Mode** (via `GCP_PROJECT` or `GOOGLE_CLOUD_PROJECT` for 300+ RPM paid tier quota). Use `python verify_environment.py` to validate your environment configuration.
- **Greptile Code Review Governance**: All Pull Requests are automatically reviewed by Greptile using repository rules defined in `.greptile/rules.md` and configuration in `.greptile/config.json`. Ensure changes pass test isolation, zero hardcoded secrets, non-blocking async I/O standards, and Antigravity 2.0 CLI-first operational guardrails.
